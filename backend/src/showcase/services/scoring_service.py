...
class ScoringService:
    ...
    def calculate_featured_score(product_status):
        ...
        views_weight = Decimal(str(SCORE_WEIGHTS['views']))
        score += min(Decimal(str(views_last_30_days / 100)) * views_weight, views_weight)

        whatsapp_weight = Decimal(str(SCORE_WEIGHTS['whatsapp_clicks']))
        score += min(
            Decimal(str(product_status.whatsapp_click_count / 50)) * whatsapp_weight,
            whatsapp_weight
        )
        ...
        # Score de disponibilité (simplifié : seulement si en stock ou non)
    ...
    def calculate_recommendation_score(product_status):
        ...
        engagement_weight = Decimal(str(RECOMMENDATION_WEIGHTS['engagement']))
        score += min(
            Decimal(str(total_engagement / 200)) * engagement_weight,
            engagement_weight
        )

        # Score de demande (simplifié : bonus si en stock et populaire)
        if product.is_in_stock:
            if product_status.view_count > 100:
                score += 20
            elif product_status.view_count > 50:
                score += 15
            elif product_status.view_count > 0:
                score += 10
            else:
                score += 5

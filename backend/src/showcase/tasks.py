...
def recalculate_product_scores(product_status_id):
    try:
        ...
        # Respecter les vedettes marquées manuellement
        # Si is_featured est déjà True, c'est une vedette manuelle - la préserver
        if instance.is_featured and featured_score < 100:
            is_featured = True
            featured_score = 100

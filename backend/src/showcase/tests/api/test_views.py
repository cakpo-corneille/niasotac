...
class CategoryAPITests(TestCase):
    ...
    def test_retrieve_category(self):
        ...
        url = reverse('category-detail', args=[self.parent.slug])
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_category_filter_by_level(self):
        ...
        url = reverse('category-list') + '?level=0'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class ProductAPITests(TestCase):
    ...
    def test_filter_by_category(self):
        ...
        url = reverse('product-list') + f'?category_slug={self.category.slug}'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_filter_by_brand(self):
        """Test filtrage par marque"""
        url = reverse('product-list') + '?brand=Sony'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_filter_by_price_range(self):
        """Test filtrage par gamme de prix"""
        url = reverse('product-list') + '?min_price=100000&max_price=200000'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_filter_by_in_stock(self):
        """Test filtrage par disponibilité"""
        url = reverse('product-list') + '?is_in_stock=true'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_search_products(self):
        """Test recherche de produits"""
        url = reverse('product-list') + '?search=Sony'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_track_whatsapp_click(self):
        ...
        url = reverse('product-track-click', args=[self.product.slug])
...
class ServiceAPITests(TestCase):
    ...
    def test_retrieve_service(self):
        ...
        url = reverse('service-detail', args=[self.service.slug])
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class SiteSettingsAPITests(TestCase):
    ...
    def test_get_settings(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['company_name'], 'NIASOTAC')
...
class PaginationTests(TestCase):
    ...
    def test_page_navigation(self):
        ...
        url = reverse('product-list') + '?page=2&page_size=10'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
